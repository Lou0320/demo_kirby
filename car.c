...
void draw(void) {
  ...
  gl4duTranslatef(0.0f, 0.0f, 0.3f * t);
  gl4duBindMatrix("mod");
  ...
  // Rue à gauche
  gl4duLoadIdentityf();
  gl4duTranslatef(-7.0f, 1.0f, -6.0f);  // Augmenté de -4.2 à -6.2
  gl4duScalef(18.0f, 18.0f, 18.0f);
  gl4duRotatef(-0.5f, 0.0f, 1.0f, 0.0f);
  gl4duSendMatrices();
  assimpDrawScene(streetModel);
  ...
  /* Voiture kirby */
  static float newX = 0.0f;  // Position X après rotation
  float carZ = -0.6f * t;  //  vitesse initiale
  
  gl4duLoadIdentityf();
  
  if(carZ <= -13.0f) {
    // Une fois que la voiture a tourné
    float timeSinceRotation = t - (-13.0f/(-0.6f)); // Ajusté pour la nouvelle vitesse
    newX = 0.3f * timeSinceRotation; // Augmenté de 0.25f à 0.3f - vitesse après rotation
    
    gl4duTranslatef(newX+0.05f, 0.35f, -13.0f);
  ...
  if(elapsedTime >= 2.0 && elapsedTime<17) {  // Changé de 3.0 à 8.0 secondes
    glEnable(GL_DEPTH_TEST);
  ...
  if(elapsedTime >= 6.0) {
    updateText(_textTexId, "Nous approchons,\nj'ai tellement hâte\nde revoir mon ami!");
  }
  if(elapsedTime >= 9.0) {
    updateText(_textTexId, "Ensuite nous partons\nfaire la fête\ntous les trois !");
  }
  glUseProgram(pId[0]);

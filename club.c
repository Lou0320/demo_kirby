...
void draw(void) {
  ...
  // Mouvement et rotation de la caméra comme une tête qui regarde
  float camera_x = sin(t * 0.3f) * 0.3f; // Réduit encore l'amplitude du mouvement latéral
  // Utilise (1 + sin()) pour garder l'angle entre 0° et 15° au lieu de -15° à 15°
  float camera_angle = (1.0f + sin(t * 0.3f)) * 7.5f; 
  
  gl4duBindMatrix("view");
  gl4duLoadIdentityf();
  
  // Position de base de la caméra avec un léger mouvement latéral
  gl4duLookAtf(camera_x, 0.6f, 1.8f,     
               // Le point ciblé se déplace moins
               camera_x + sin(t * 0.3f) * 1.5f, -5.0f, -12.0f,   
               0.0f, 1.0f, 0.0f);         
  gl4duRotatef(camera_angle, 0.0f, 1.0f, 0.0f); // Ajoute une rotation
  gl4duTranslatef(0.0f, 0.0f, 0.8f);
  gl4duBindMatrix("mod");

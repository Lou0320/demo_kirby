...
// Ajout des positions de départ et d'arrivée
static struct {
    float startX, startY, startZ;
    float endX, endY, endZ;
    float duration;  // durée de la transition en secondes
    float startTime; // temps de début de la transition
} carTransition = {
    -0.7f, 2.1f, -3.0f,  // position de départ
    -0.4f, 2.3f, -3.0f,  // position finale
    4.0f,                 // durée de 2 secondes
    0.0f                  // sera initialisé au premier rendu
};
...
void draw(void) {
  ...
  /*Voiture kirby*/
  if(t > 2.0) { 
    float progress = (t - 2.0) / 4.0;  
    if(progress > 1.0f) progress = 1.0f;
    
    // Interpolation linéaire entre les positions
    float currentX = carTransition.startX + (carTransition.endX - carTransition.startX) * progress;
    float currentY = carTransition.startY + (carTransition.endY - carTransition.startY) * progress;
    float currentZ = carTransition.startZ + (carTransition.endZ - carTransition.startZ) * progress;
    
    gl4duLoadIdentityf();
    gl4duTranslatef(currentX, currentY, currentZ);
    gl4duScalef(0.2f, 0.3f, 0.3f);
    gl4duRotatef(90.0f, 1.0f, 0.0f, 0.0f);
    gl4duRotatef(115.0f, 0.0f, 1.0f, 0.0f);
    gl4duSendMatrices();
    glUniform4f(glGetUniformLocation(pId[0], "couleur"), 1.0f, 1.0f, 1.0f, 1.0f);

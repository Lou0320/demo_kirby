...
HEADERS = animations.h audioHelper.h assimp.h obj_loader.h
SOURCES = window.c audioHelper.c animations.c intro.c car.c assimp.c transform.c duck.c zoom.c map1.c map2.c club.c dancing.c dancezoom.c credits.c logo.c
MSVCSRC = $(patsubst %,<ClCompile Include=\"%\\\" \\/>,$(SOURCES))
OBJ = $(SOURCES:.c=.o) 
DOXYFILE = documentation/Doxyfile
VSCFILES = $(PROGNAME).vcxproj $(PROGNAME).sln
EXTRAFILES = COPYING $(wildcard shaders/*.?s images/*) $(shell find models -type f) $(VSCFILES) audios/mathis.mp3 DejaVuSans-Bold.ttf
...
LDFLAGS  += -lGL4Dummies $(shell sdl2-config --libs) -lSDL2_mixer -lSDL2_image -lSDL2_ttf -lassimp
all: $(PROGNAME)
$(PROGNAME): $(OBJ)
